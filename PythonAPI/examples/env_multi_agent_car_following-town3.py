...
class CarFollowing(object):
    ...
    def reset(self):
        ...
        #spawn_point_front.location.x = ego_location.x
        #spawn_point_front.location.y = ego_location.y-10
        spawn_point_front.location.z = 0.01
        ...
        #spawn_point_front.location.x = ego_location.x
        #spawn_point_front.location.y = ego_location.y-10
        spawn_point_back.location.z = 0.01
        ...
        if self.back_vehicle is not None:
            self.destroy()
        self.back_vehicle = self.world.spawn_actor(bp_back, spawn_point_back)
        
        self.world.tick()
        
        self.front_agent = BasicAgent(self.front_vehicle, target_speed = 15 * 3.6)
        self.agent = BasicAgent(self.ego_vehicle, target_speed = 15 * 3.6)
        self.back_agent = BasicAgent(self.back_vehicle, target_speed = 15 * 3.6)
        ...
        lat_list = [225, 225, 225]
        long_list = [-375, -368, -364]
    ...
    def step(self, action):
        ...
        if self.mode == 'steer':
            self.control.steer = self.front_control.steer *0.0001
        ...
        if self.brake_idx < self.count <= self.brake_idx+ 50:
            print('braking event')
            ...
            if self.count == self.brake_idx+ 50:
        ...
        done = collision or finish or self.count>4000/12*self.frame
