...
class CarFollowing(object):
    def __init__(self, enabled_obs_number=3, vehicle_type = 'random',
                 ...
                 conservative_surrounding = False, frame=12, port=2000, mode='auto'):
    ...
    def reset(self):
        ...
        spawn_point_ego.location.x = 235
        spawn_point_ego.location.y = -371
        spawn_point_ego.location.z = 0.1
        spawn_point_ego.rotation.yaw = 0
        ...
        self.back_vehicle = self.world.spawn_actor(bp_ego, spawn_point_back)
        ...
        self.front_agent = BasicAgent(self.front_vehicle, target_speed = 15 * 3.6, opt_dict={'ignore_stop_signs': True, 'ignore_vehicles': True})
        self.agent = BasicAgent(self.ego_vehicle, target_speed = 0 * 3.6)
        self.back_agent = BasicAgent(self.back_vehicle, target_speed = 15 * 3.6, opt_dict={'ignore_stop_signs': True, 'ignore_vehicles': True})
        ...
        try:
            with open(output_file, 'w', newline='') as csvfile:
                ...
                writer.writerow(['time', 'velocity_ego', 'velocity_front', 'velocity_back', 'distance_front','distance_back','pedal','steer'])        
    ...
    def step(self, action):
        ...
        try:
            with open(output_file, 'a', newline='') as csvfile:
                ...
                # Write the variables as a single row in the CSV file
                writer.writerow([time.perf_counter(), v_ego, v_front, v_back, dis_front, dis, self.control.throttle - self.control.brake, self.control.steer ])
        except IOError as e:
            print("Error:", e)
