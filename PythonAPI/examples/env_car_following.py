...
'''
try:
    sys.path.append(glob.glob(r'E:\UTS\code\CARLA_0.9.9.4\WindowsNoEditor\PythonAPI\carla\dist\carla-*%d.%d-%s.egg' % (
...
class CarFollowing(object):
    def __init__(self, enabled_obs_number=6,  vehicle_type = 'random',
        ...
        pygame.display.set_caption('Car Following Scenario')
    ...
    def reset(self):
        ...
        self.agent = BasicAgent(self.ego_vehicle, target_speed = 25.5 * 3.6)
        #self.agent.set_destination((-13, -80, spawn_point_ego.location.z+1))
        
        self.ego_vehicle.set_target_velocity(carla.Vector3D(0, 25, 0))
        ...
        lat_list = [-16.5, -13, -9.5, -9.5, -13, -16.5]
        long_list = [-170, -1750, -170, -160, -100, -158]
        
        # Set the waypoints for the ego vehicle to follow the lane
        self.lane_waypoints_ego = self._get_lane_waypoints(self.ego_vehicle)
        self.lane_following_index_ego = 0

        # Set the waypoints for each surrounding vehicle to follow the lane
        self.lane_waypoints_surrounding = []            
        ...
        for index in range(self.enabled_obs):
            try:
                ...
                self.obs_list[index].set_target_velocity(carla.Vector3D(0, 25, 0))
            except:
                self.enabled_obs -= 1
                pass
        for obs_agent in self.obs_list:
            lane_waypoints_surrounding = self._get_lane_waypoints(obs_agent)
            self.lane_waypoints_surrounding.append(lane_waypoints_surrounding)
            obs_agent.lane_following_index = 0
    ...
    def step(self,action):
        ...
        for index in range(self.enabled_obs):
            ...
            obs_command.steer = 0
            obs_velocity_diff = 25 - self.obs_list[index].get_velocity().y
            ...
            self.ego_vehicle.apply_control(obs_command)
        '''
        # Implement lane following behavior for the ego vehicle
        #self.lane_waypoints_ego = self._get_lane_waypoints(self.ego_vehicle)
        self.lane_following_index_ego += 1
        #if self.lane_following_index_ego < len(self.lane_waypoints_ego):
        #    target_location = self.lane_waypoints_ego[self.lane_following_index_ego]
        #    self.agent.set_destination((target_location.x, target_location.y, target_location.z))
        #else:
        #self.agent.set_destination((self.lane_waypoints_ego[-1].x, self.lane_waypoints_ego[-1].y, self.lane_waypoints_ego[-1].z))
        #print(self.lane_waypoints_ego[-1].x)
        '''
        # Implement lane following behavior for each surrounding vehicle
        for i, obs_agent in enumerate(self.obs_list):
            obs_agent.lane_following_index += 1
            if obs_agent.lane_following_index < len(self.lane_waypoints_surrounding[i]):
                target_location = self.lane_waypoints_surrounding[i][obs_agent.lane_following_index]
                obs_agent.set_destination((target_location.x, target_location.y, target_location.z))
            else:
                obs_agent.set_destination((self.lane_waypoints_surrounding[i][-1].x, self.lane_waypoints_surrounding[i][-1].y, self.lane_waypoints_surrounding[i][-1].z))
        '''
        ...
        finish = self.ego_vehicle.get_location().y > 8000
    ...
    # Define a helper function to get lane waypoints for a given vehicle
    def _get_lane_waypoints(self, vehicle):
        map = self.world.get_map()
        waypoints = map.get_waypoint(vehicle.get_location())
        
        # Get lane waypoints by sampling waypoints ahead of the vehicle
        lane_waypoints = []
        num_waypoints = 1
        distance = 2.0  # Distance between consecutive waypoints in meters
        
        for _ in range(num_waypoints):
            distance = distance + 2.0
            waypoint = waypoints.next(distance)[0]
            lane_waypoints.append(waypoint.transform.location)
