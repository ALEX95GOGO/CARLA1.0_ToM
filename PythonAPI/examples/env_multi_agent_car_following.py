...
class CarFollowing(object):
    def __init__(self, enabled_obs_number=2, vehicle_type = 'random',
                 ...
                 conservative_surrounding = False, frame=12, port=2000, mode='auto', town=4, recorder_filename='test'):
        ...
        self.town = town
        self.frame = frame
        print("Recording on file: %s" % self.client.start_recorder(recorder_filename))
    ...
    def reset(self):
        ...
        self.ego_sensor = DReyeVRSensor(self.world)

        selfhost = "192.168.86.123"
        roshost = "192.168.86.33"
        rosport = 11311
        import pdb; pdb.set_trace()
        if rospy is not None:
            # tunable parameters for your configuration
            IP_SELF = selfhost  # where the rosnode is being run (here)
            ...
            IP_ROSMASTER = roshost
            PORT_ROSMASTER = rosport
            self.pub = self.init_ros_pub(IP_SELF, IP_ROSMASTER, PORT_ROSMASTER)
        ...
        for idx, w in enumerate(waypoints):
            plt.plot(w.location.x, w.location.y, 'x')  # Plot as 'x' symbol
            plt.text(w.location.x, w.location.y, str(idx), color='red')  # Label with index
        ...
        if self.town == 4:
            ...
            spawn_point_front.location.x = 235
        ...
        self.agent = BasicAgent(self.ego_vehicle, target_speed = 0 * 3.6)
        #self.agent = AgentWrapper(self.agent)
        self.front_agent = BasicAgent(self.front_vehicle, target_speed = 0 * 3.6, opt_dict={'ignore_stop_signs': True, 'ignore_vehicles': True, 'ignore_traffic_lights': True})
        self.back_agent = BasicAgent(self.back_vehicle, target_speed = 0 * 3.6, opt_dict={'ignore_stop_signs': True, 'ignore_vehicles': True, 'ignore_traffic_lights': True})
        ...
        initial_front_velocity = carla.Vector3D(0, 0, 0)
        self.front_vehicle.set_target_velocity(initial_front_velocity)

        initial_back_velocity = carla.Vector3D(0, 0, 0)
        self.back_vehicle.set_target_velocity(initial_back_velocity)
        ...
        state = np.array([dis, dis_front, v_ego])
        #next_observation_tensor = torch.tensor(next_observation, dtype=torch.float32)

        output_file = "output{}.csv".format(self.now)
        try:
            with open(output_file, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['time', 'velocity_ego', 'velocity_front', 'velocity_back', 'distance_front','distance_back','throttle','brake','steer', 'lane_depature', \
                'acc_ego_x','acc_ego_y', 'acc_front_x', 'acc_front_y','acc_back_x','acc_back_y'])        
                # Write the variables as a single row in the CSV file
                #writer.writerow([v_ego, dis_front, dis])
        except IOError as e:
            print("Error:", e)
        return state
    ...
    def step(self, action):
        ...
        def publish_and_print(data):
            self.ego_sensor.update(data)
            if rospy is not None:
                msg: String = self.create_ros_msg(self.ego_sensor)
                self.pub.publish(msg)  # publish to ros master
            print(self.ego_sensor.data)  # more useful print here (contains all attributes)
            # print(data) # this print is defined in LibCarla/source/carla/data/DReyeVREvent.h

        # subscribe to DReyeVR sensor
        self.ego_sensor.ego_sensor.listen(publish_and_print)
        ...
        waypoint01 = self.map.get_waypoint(self.ego_vehicle.get_location(),project_to_road=True, lane_type=(carla.LaneType.Driving | carla.LaneType.Sidewalk))
        waypoint02 = self.map.get_waypoint(self.front_vehicle.get_location(),project_to_road=True, lane_type=(carla.LaneType.Driving | carla.LaneType.Sidewalk))
        waypoint03 = self.map.get_waypoint(self.back_vehicle.get_location(),project_to_road=True, lane_type=(carla.LaneType.Driving | carla.LaneType.Sidewalk))
        ...
        cross_product = np.cross(ego_forward_vector, vector_to_lane_center)

        # Determine if the lane is on the right or left of the ego car
        if cross_product > 0:
            print("The lane is on the right of the ego car.")
            lane_side = -1
        elif cross_product < 0:
            print("The lane is on the left of the ego car.")
            lane_side = 1
        else:
            print("The ego car is on the center of the lane.")
            lane_side = 0


        lane_departure_dis =((lane_center.transform.location.x - self.ego_vehicle.get_location().x)**2+(lane_center.transform.location.y - self.ego_vehicle.get_location().y)**2)**(1/2)

        if self.town == 4:
            if lane_center.lane_id !=-2:
                lane_departure_dis = 3.5
                #lane_departure_dis = np.abs(lane_center.lane_id+2)*lane_center.lane_width + lane_side*((lane_center.transform.location.x - self.ego_vehicle.get_location().x)**2+(lane_center.transform.location.y - self.ego_vehicle.get_location().y)**2)**(1/2)

        if self.town == 7:
            if lane_center.lane_id !=-1:
                lane_departure_dis = 3.5
            #    lane_departure_dis = np.abs(lane_center.lane_id+1)*lane_center.lane_width-((lane_center.transform.location.x - self.ego_vehicle.get_location().x)**2+(lane_center.transform.location.y - self.ego_vehicle.get_location().y)**2)**(1/2)

                #print(lane_departure_dis) 

        simulation_time = self.world.get_snapshot().timestamp.elapsed_seconds

        if simulation_time > 20 and simulation_time <20.5:
            self.front_agent = BasicAgent(self.front_vehicle, target_speed = 12 * 3.6, opt_dict={'ignore_stop_signs': True, 'ignore_vehicles': True, 'ignore_traffic_lights': True})
            self.back_agent = BasicAgent(self.back_vehicle, target_speed = 12 * 3.6, opt_dict={'ignore_stop_signs': True, 'ignore_vehicles': True, 'ignore_traffic_lights': True})
            
            if self.town == 4 or self.town ==6:
                #self.front_agent = BasicAgent(self.front_vehicle, target_speed = 20 * 3.6, opt_dict={'ignore_stop_signs': True, 'ignore_vehicles': True, 'ignore_traffic_lights': True})
                #self.back_agent = BasicAgent(self.back_vehicle, target_speed = 20 * 3.6, opt_dict={'ignore_stop_signs': True, 'ignore_vehicles': True, 'ignore_traffic_lights': True})
                self.front_agent = BasicAgent(self.front_vehicle, target_speed = 20 * 3.6)
                self.back_agent = BasicAgent(self.back_vehicle, target_speed = 20 * 3.6,)
        ...
        for value in self.random_list:
            if value + 50 < self.count < value + 100:
                #print('braking event')
                self.front_control.brake = 0.75
                self.front_control.throttle = 0
                self.back_control.brake = 0.75
                self.back_control.throttle = 0
        '''
        
        if v_ego > 10 and self.count>=self.brake_idx:
            self.brake_flag = 1
        ...
        if self.brake_idx <= self.count <= self.brake_idx+ 80:
            if self.brake_flag == 1:
                ...
                if self.count == self.brake_idx+ 80:
        ...
        next_observation = [dis_front-20, dis_ego-20, v_ego]
        ...
        #done = collision or finish or self.count>8000/12*self.frame or (dis_ego > dis and self.count>50) 
        done = 0
        ...
        reward = - collision*1000 + punishment
        # - 0.5*(v_ego<0.2)
        #print(reward)
        ...
        try:
            with open(output_file, 'a', newline='') as csvfile:
                writer = csv.writer(csvfile)
                #writer.writerow(['velocity_ego', 'velocity_front', 'velocity_back', 'distance_front','distance_back','pedal','steer'])
                # Write the variables as a single row in the CSV file
                writer.writerow([time.perf_counter(), v_ego, v_front, v_back, dis_front, dis, self.control.throttle, self.control.brake, self.control.steer, lane_departure_dis, self.ego_vehicle.get_acceleration().x, self.ego_vehicle.get_acceleration().y, self.front_vehicle.get_acceleration().x, self.front_vehicle.get_acceleration().y, self.back_vehicle.get_acceleration().x, self.back_vehicle.get_acceleration().y])
        except IOError as e:
            print("Error:", e)
