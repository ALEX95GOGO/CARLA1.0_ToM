...
def main():
    ...
    try:
        ...
        current_w = m.get_waypoint(vehicle.get_location())

        while True:

            next_w = m.get_waypoint(vehicle.get_location(), lane_type=carla.LaneType.Driving | carla.LaneType.Shoulder | carla.LaneType.Sidewalk )
            ...
            if next_w.id != current_w.id:
                ...
                debug.draw_string(current_w.transform.location, str('%15.0f km/h' % (3.6 * math.sqrt(vector.x**2 + vector.y**2 + vector.z**2))), True, orange, 60)
