...
class CuttingInActiveObject(BasicScenario):
    '''
    switch: 
    0: 90 degree pedestrian cross
    1: 90 degree pedestrian not cross
    2: 60 degree pedestrian cross
    3: 60 degree pedestrian not cross
    '''
    ...
    def _initialize_actors(self, config):
        ...
        if config.switch == 0 or config.switch == 1:
            offset = {'yaw': 270, 'z': 0.65, 'k': 0.75}
        if config.switch == 2 or config.switch == 3:
    ...
    def _create_behavior(self):
        ...
        if self.config.switch == 0 or self.config.switch == 2:
