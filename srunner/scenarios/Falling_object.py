...
from core.simulators.srunner.scenariomanager.scenarioatomics.atomic_behaviors import (ActorTransformSetter,
                                                                      ActorDestroy,
                                                                      KeepVelocity,
                                                                      StopVehicle,
                                                                      WaypointFollower,
...
class FallingObject(BasicScenario):
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self.second_vehicle_distance = 22
        self.pedestrian_distance = 19.25
        self.pedestrain_end_distance = 42.5
        self._destory_actors_threshold = 8
        self._num_lane_changes = 0
        self._adversary_type = 'walker.*'
        self._adversary_speed = 1.25#3.0  # Speed of the adversary [m/s]
        self._reaction_time = 8.0#0.8  # Time the agent has to react to avoid the collision [s]
        ...
        super(FallingObject, self).__init__("FallingObject",
                                                    ego_vehicles,
                                                    config,
                                                    world,
                                                    debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        """
        custom_foggy_weather = carla.WeatherParameters(
        cloudiness=80.0,
        precipitation=0.0,
        fog_density=90.0,
        fog_distance=10.0,
        ...
        self.world.set_weather(custom_foggy_weather)
        
        waypoint = self._reference_waypoint
        wp_next = waypoint.get_right_lane()

        self.wp_first_object, _ = get_waypoint_in_distance(waypoint, self.first_vehicle_distance)
        ...
        self.wp_pedestrian_end, _ = get_waypoint_in_distance(wp_next, self.pedestrain_end_distance)
        self._collision_wp, _ = get_waypoint_in_distance(self._reference_waypoint, self.pedestrian_distance)
        
        # Object choices (random selection for variation)
        #object_choices = ["static.prop.box", "static.prop.pallet", "static.prop.rock"]
        #object_choices = ["static.prop.box01", "static.prop.travelcase","static.prop.shoppingtrolley"]
        object_choices = ["static.prop.box01", "static.prop.streetbarrier", "static.prop.shoppingcart", "static.prop.shoppingtrolley", "static.prop.constructioncone", "static.prop.trafficwarning", "static.prop.trafficwarning"]
        random.seed()
        object_model = random.choice(object_choices)

        # Initial transform (above the road, so it drops)
        object_transform = self.wp_first_object.transform

        object_blueprint = self.world.get_blueprint_library().find(object_model)
        self.object = self.world.spawn_actor(object_blueprint, object_transform)
        
        
        object_choices = ["static.prop.colacan", "static.prop.dirtdebris01", "static.prop.ironplank"]
        object_model = random.choice(object_choices)

        # Initial transform (above the road, so it drops)
        object_transform = self.wp_second_vehicle.transform

        object_blueprint = self.world.get_blueprint_library().find(object_model)
        self.object = self.world.spawn_actor(object_blueprint, object_transform)
        #self.other_actors.append(self.object)
        ...
        offset = {'yaw': 0, 'z': 0.5, 'k': 2.0}
        self._adversary_transform = self._get_sidewalk_transform(sidewalk_waypoint, offset)
        self._adversary_transform_end = self._get_sidewalk_transform(self.wp_pedestrian_end, offset)
        offset = {'yaw': 270, 'z': 0.5, 'k': 1.5}
        self._adversary_transform_turn = self._get_sidewalk_transform(self.wp_pedestrian_end, offset)
        
        offset = {'yaw': 270, 'z': 0.5, 'k': 0.8}
        self._adversary_transform_turn2 = self._get_sidewalk_transform(self.wp_pedestrian_end, offset)
        print(self._adversary_transform_turn)
        print(self._adversary_transform)
        ...
        self.falling_object_transform = carla.Transform(carla.Location(self.wp_first_vehicle.transform.location.x,
                                                                  self.wp_first_vehicle.transform.location.y,
                                                                  self.wp_first_vehicle.transform.location.z + 3),
        ...
        #first_vehicle = CarlaDataProvider.request_new_actor("static.prop.box01", second_vehicle_temp_transform)
    ...
    def _create_behavior(self):
        ...
        #start_first_vehicle_transform = ActorTransformSetter(self.other_actors[1], self.falling_object_transform)
        ...
        second_vehicle_sequence.add_child(KeepVelocity(
            self.other_actors[2], 5,
            distance=10, name="AdversaryCrossing2"))
        second_vehicle_sequence.add_child(StopVehicle(self.other_actors[2], 1.0))
