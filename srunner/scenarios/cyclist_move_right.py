...
class CyclistMoveRight(BasicScenario):
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
                 timeout=90):
        self._wmap = CarlaDataProvider.get_map()
        self._reference_waypoint = self._wmap.get_waypoint(config.trigger_points[0].location)
        self._ego_route = CarlaDataProvider.get_ego_vehicle_route()
        self.first_vehicle_distance = 1#15
        self.second_vehicle_distance = 10#24
        ...
        super(CyclistMoveRight, self).__init__("CyclistMoveRight",
                                                    ego_vehicles,
                                                    config,
                                                    world,
                                                    debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        wp_next_slight_right = self.wp_first_vehicle
        print(wp_next_slight_right.transform.rotation)
        #wp_next_slight_right.transform.rotation.yaw-=60
        
        # Create a new Transform object with modified yaw
        new_transform = carla.Transform(
                location=wp_next_slight_right.transform.location,  # Keep same location
                rotation=carla.Rotation(
                        pitch=wp_next_slight_right.transform.rotation.pitch,  # Preserve pitch
                        yaw=wp_next_slight_right.transform.rotation.yaw + 10,  # Add 10 degrees
                        roll=wp_next_slight_right.transform.rotation.roll  # Preserve roll
                )
        )
        self.transfrom_slight_right = new_transform
        #wp_next_slight_right.transform = new_transform
        ...
        # Generate waypoints moving slightly to the right
        '''
        print(self.wp_first_vehicle)
        self.wp_first_vehicle = []
        for i in range(10):
            next_wp = carla.Transform(
                carla.Location(x=wp_next.transform.location.x + i * 1.0,  # Move forward
                               y=wp_next.transform.location.y + i * 0.2,  # Slightly right
                               z=wp_next.transform.location.z),
                wp_next.transform.rotation
            )
            #print(next_wp.transform.location)
            self.wp_first_vehicle.append(WaypointFollower(self.wp_first_vehicle, target_speed=5, plan=[next_wp]))
		'''
        ...
        first_vehicle_temp_transform = carla.Transform(carla.Location(self.transfrom_slight_right.location.x,
        ...
        first_vehicle = CarlaDataProvider.request_new_actor('vehicle.diamondback.century', first_vehicle_temp_transform)
        second_vehicle = CarlaDataProvider.request_new_actor('vehicle.diamondback.century', second_vehicle_temp_transform)
    ...
    def _create_behavior(self):
        ...
        sink_parallel.add_child(ActorSink(self.transfrom_slight_right.location, self._destory_actors_threshold))
        ...
        start_first_vehicle_transform = ActorTransformSetter(self.other_actors[2], self.transfrom_slight_right)
