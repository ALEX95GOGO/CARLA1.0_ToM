...
class CuttingInActiveObject(BasicScenario):
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        #self._walker_idle_time = 0.15
        self._adversary_type = 'walker.*'
        self._adversary_speed = 1.6#3.6  # Speed of the adversary [m/s]
        self._reaction_time = 0.35  # Time the agent has to react to avoid the collision [s]
        #self._reaction_time = 1.0  # Time the agent has to react to avoid the collision [s]
        ...
        #self._traffic_light = CarlaDataProvider.get_next_traffic_light(config.trigger_points[0].location)
    ...
    def _create_behavior(self):
        ...
        speed_duration = 0.5 * collision_duration #2.0 * collision_duration
        speed_distance = 0.5 * collision_distance #2.0 * collision_distance
        ...
        trigger_adversary = py_trees.composites.Parallel(
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE, name="TriggerAdversaryStart1")
        trigger_adversary.add_child(InTimeToArrivalToLocation(
            self.ego_vehicles[0], 1.5*collision_time_trigger, collision_location))
        trigger_adversary.add_child(InTriggerDistanceToLocation(
            self.ego_vehicles[0], collision_location, 1.5*self._min_trigger_dist))
        pedestrian_sequence.add_child(trigger_adversary)
        pedestrian_start_event = py_trees.composites.Parallel(
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE, name="PedestrianStart1")
        pedestrian_start_event.add_child(KeepVelocity(
            self.other_actors[0], self._adversary_speed,
            duration=speed_duration, distance=speed_distance, name="AdversaryCrossing01"))
        pedestrian_start_event.add_child(KeepVelocity(
            self.other_actors[1], self._adversary_speed,
            duration=speed_duration, distance=speed_distance, name="AdversaryCrossing11"))
        pedestrian_sequence.add_child(pedestrian_start_event)
        
        # comment this to disable pedestrian behavior
        trigger_keep_velocity.add_child(pedestrian_sequence)


        # Wait until ego is close to the adversary
        trigger_adversary = py_trees.composites.Parallel(
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE, name="TriggerAdversaryStart2")
        trigger_adversary.add_child(InTimeToArrivalToLocation(
        ...
        pedestrian_sequence.add_child(trigger_adversary)
        pedestrian_start_event = py_trees.composites.Parallel(
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE, name="PedestrianStart2")
        pedestrian_start_event.add_child(KeepVelocity(
            self.other_actors[0], -self._adversary_speed,
            duration=speed_duration, distance=speed_distance, name="AdversaryCrossing02"))
        pedestrian_start_event.add_child(KeepVelocity(
            self.other_actors[1], -self._adversary_speed,
            duration=speed_duration, distance=speed_distance, name="AdversaryCrossing12"))
        pedestrian_sequence.add_child(pedestrian_start_event)
        ...
        first_vehicle_sequence.add_child(KeepVelocity(
            ...
            distance=first_vehicle_speed_distance-1, name="AdversaryCrossing1"))
        ...
        first_vehicle_sequence.add_child(Idle(45))
