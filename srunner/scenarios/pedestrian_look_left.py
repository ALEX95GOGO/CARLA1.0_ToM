...
class PedestrianLookLeft(BasicScenario):
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super(PedestrianLookLeft, self).__init__("PedestrianLookLeft",
                                                    ego_vehicles,
                                                    config,
                                                    world,
                                                    debug_mode,
    ...
    def _create_behavior(self):
		...
		# Get adversary's current waypoint
        adversary_location = self.other_actors[0].get_location()
        #map = self.world.get_map()
        adversary_waypoint = self._wmap.get_waypoint(adversary_location)

        # Define a FollowWaypoints behavior dynamically
        turn_waypoints = [
            self._adversary_transform_end.location,  # Move forward
            self._adversary_transform_turn.location # Turn
            #next_wp_turn.transform  # Turn 90 degrees right
        ]

        turn_smoothly = WaypointFollower(self.other_actors[0], 1.6, turn_waypoints)
        sequence.add_child(turn_smoothly)
        
        turn_waypoints2 = [
			self._adversary_transform_turn.location,
            self._adversary_transform_turn2.location # Turn
            #next_wp_turn.transform  # Turn 90 degrees right
        ]
        
        turn_smoothly2 = WaypointFollower(self.other_actors[0], 0.0, turn_waypoints2)
        #sequence.add_child(turn_smoothly2)
        
        #sequence.add_child(KeepVelocity(
        #    self.other_actors[0], 0.1,
        #    duration=5, distance=0.5, name="AdversaryCrossing"))
